...
switch ($method) {
    case 'GET':
        ...
        $sql = "SELECT DISTINCT `rmc`.*, `players`.`displayName` FROM `rmc` INNER JOIN `players` ON `rmc`.`accountId` = `players`.`accountId`".$whereSQL." GROUP BY `rmc`.`accountId` ORDER BY `rmc`.`goals` DESC, `rmc`.`belowGoals` DESC";
